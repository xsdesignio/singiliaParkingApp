...
export class communicationCreator {
    constructor () {
        this.communicationEncoder = new PrinterCommunicationEncoder();
    }

    createTicketChunks(ticket_data) {
        // Logo is divided in two
        const logo = this.communicationEncoder.getSingiliaLogo()
        const title = this.communicationEncoder.getEncodedTitle("Servicio Municipal \nEstacionamiento Regulado")
        const encoded_ticket_data = this.communicationEncoder.getEncodedDict(ticket_data)
        const singilia_barba_info = this.communicationEncoder.getSingiliaInfo()
        const legal_info = this.communicationEncoder.getBulletinLegalInfo()


        const chunks = [logo[0], logo[1], title, encoded_ticket_data, singilia_barba_info, legal_info]

        return chunks

    }

    createBulletinChunks(bulletin_data, available_bulletins) {
        const logo = this.communicationEncoder.getSingiliaLogo()
        const title = this.communicationEncoder.getEncodedTitle("Boletín \nEstacionamiento Regulado")
        const encoded_bulletin_data = this.communicationEncoder.getEncodedDict(bulletin_data)
        const prices_table = this.communicationEncoder.getEncodedPricesTable(available_bulletins)
        const singilia_barba_info = this.communicationEncoder.getSingiliaInfo()
        const legal_info = this.communicationEncoder.getBulletinLegalInfo()
 
        const chunks = [logo, title, encoded_bulletin_data, prices_table, singilia_barba_info, legal_info]
        return chunks
    }

    createAnullationBulletinChunks(bulletin_data) {
        
        const logo = this.communicationEncoder.getSingiliaLogo()
        const title = this.communicationEncoder.getEncodedTitle("Anulación de Boletín")
        const encoded_bulletin_data = this.communicationEncoder.getEncodedDict(bulletin_data)
        const singilia_barba_info = this.communicationEncoder.getSingiliaInfo()
        const legal_info = this.communicationEncoder.getBulletinLegalInfo()

        const chunks = [logo, title, encoded_bulletin_data, singilia_barba_info, legal_info]
        return chunks
    }
}
